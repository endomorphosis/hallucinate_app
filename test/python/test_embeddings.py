...
# Import the IPFS Embeddings integration module
try:
    from hallucinate_app.python.hallucinate_app.ipfs_embeddings import IPFSEmbeddings, ipfs_embeddings
except ImportError:
    # Try alternative import path
    try:
        from hallucinate_app.hallucinate_app.ipfs_embeddings import IPFSEmbeddings, ipfs_embeddings
    except ImportError:
        print("Failed to import IPFS Embeddings module. Make sure the module exists and paths are correct.")
        sys.exit(1)

class TestIPFSEmbeddingsIntegration(unittest.TestCase):
    """
    Test the IPFS Embeddings integration layer.
    ...
    @classmethod
    def setUpClass(cls):
        # Initialize the IPFS Embeddings integration module with test resources
        cls.resources = {}
        cls.metadata = {'test': True, 'cache_dir': os.path.join(os.path.expanduser('~'), '.cache', 'embeddings_test')}
        cls.ipfs_embeddings = IPFSEmbeddings(cls.resources, cls.metadata)
        ...
        cls.loop.run_until_complete(cls.ipfs_embeddings.init())
        
        # Store module availability for test decisions
        cls.has_external_module = (
            cls.ipfs_embeddings.modules.get("ipfs_embeddings_py") is not None
        )
    ...
    def test_initialization(self):
        ...
        result = self.loop.run_until_complete(self.ipfs_embeddings.init())
    ...
    def test_module_detection(self):
        ...
        self.assertIsInstance(self.ipfs_embeddings.modules, dict, "Integration module should track external modules in a dict")
        self.assertIn("ipfs_embeddings_py", self.ipfs_embeddings.modules, "Integration module should track ipfs_embeddings_py")
    ...
    def test_method_forwarding(self):
        ...
        if not self.has_external_module:
            # Test with generate_embedding call
            test_text = "This is a test sentence for embedding."
            result = self.loop.run_until_complete(self.ipfs_embeddings.generate_embedding(test_text))
    ...
    def test_module_test_method(self):
        ...
        test_result = self.ipfs_embeddings.test()
        ...
        self.assertEqual(
            test_result.get('capabilities', {}).get('ipfs_embeddings_py', False),
    ...
    def test_ipfs_operations(self):
        ...
        # If no external modules are available, method calls should return error objects
        if not self.has_external_module:
            # Test with save_embeddings_to_ipfs call
            test_embeddings = [{"id": 1, "embedding": [0.1, 0.2, 0.3]}]
            result = self.loop.run_until_complete(
                self.ipfs_embeddings.save_embeddings_to_ipfs(test_embeddings)
            )
            self.assertIsInstance(result, dict, "Method should return dict when external modules unavailable")
            self.assertIn("error", result, "Method should return error when external modules unavailable")
            
            # Test with load_embeddings_from_ipfs call
            result = self.loop.run_until_complete(
                self.ipfs_embeddings.load_embeddings_from_ipfs("test-cid")
            )
            self.assertIsInstance(result, dict, "Method should return dict when external modules unavailable")
            self.assertIn("error", result, "Method should return error when external modules unavailable")
            return
        ...
        try:
            ...
            # Test loading from IPFS (using a fake CID)
...
if __name__ == "__main__":
    ...
    test_suite = test_loader.loadTestsFromTestCase(TestIPFSEmbeddingsIntegration)
