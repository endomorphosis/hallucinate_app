...
const VAI_571_GATE_FIXTURE = path.join(__dirname, 'fixtures', 'vai-571-daemon-launch-health-gate.json');
...
test.describe('MGW-535 daemon launch health Playwright gate', () => {
  test('keeps the daemon launch validation gate fixture in parity with the manager', () => {
    ...
    expect(gate.objective_gap_receipts).toContain('data/virtual_ai_os/discovery/2026-07-04-vai-571-objective-gap-b023c8de5b69.md');
    ...
    expect(gate.discovery_receipts).toContain('data/virtual_ai_os/discovery/2026-07-04-vai-571-daemon-launch-health-gate.md');
  ...
  test('closes the MGW-551 daemon launch objective gap with the current Playwright gate', () => {
    ...
    expect(launchPlan.every((entry: any) => (
      ...
      entry.launch_validation_gates.some((candidate: any) => candidate.task_id === 'VAI-571') &&
  ...
  test('binds the VAI-538, VAI-540, VAI-549, VAI-555, VAI-557, VAI-565, VAI-568, and VAI-571 objective gap receipts to the daemon launch Playwright gate', () => {
    const manager = new MCPDaemonManager();
    const gate = manager.getDaemonLaunchValidationGate();
    const gates = manager.getDaemonLaunchValidationGates();
    const fixtures = [
      ...
      {
        taskId: 'VAI-571',
        fixturePath: VAI_571_GATE_FIXTURE,
        gapReceipt: 'data/virtual_ai_os/discovery/2026-07-04-vai-571-objective-gap-b023c8de5b69.md',
        launchReceipt: 'data/virtual_ai_os/discovery/2026-07-04-vai-571-daemon-launch-health-gate.md',
        daemonLaunchCommand: 'test ! -f hallucinate_app/package.json || npm --prefix hallucinate_app run test:e2e -- daemon-launch-health.spec.ts'
      }
    ];
