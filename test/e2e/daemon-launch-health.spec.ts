...
test.describe('MGW-535 daemon launch health Playwright gate', () => {
  ...
  test('binds the VAI-538, VAI-540, VAI-549, VAI-555, VAI-557, VAI-565, VAI-568, VAI-574, VAI-577, VAI-580, VAI-583, VAI-586, VAI-589, VAI-593, VAI-596, VAI-599, VAI-602, and VAI-605 objective gap receipts to the daemon launch Playwright gate', () => {
    const manager = new MCPDaemonManager();
    const gate = manager.getDaemonLaunchValidationGate();
    const gates = manager.getDaemonLaunchValidationGates();
    const fixtures = [
