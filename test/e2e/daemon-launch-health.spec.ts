...
const VAI_658_GATE_FIXTURE = path.join(__dirname, 'fixtures', 'vai-658-daemon-launch-health-gate.json');
const VAI_658_ATTEMPT_RECEIPT = path.join(
  repoRoot,
  'data',
  'virtual_ai_os',
  'discovery',
...
const VAI_660_GATE_FIXTURE = path.join(__dirname, 'fixtures', 'vai-660-daemon-launch-health-gate.json');
const VAI_660_ATTEMPT_RECEIPT = path.join(
  repoRoot,
  'data',
  'virtual_ai_os',
  'discovery',
...
function expectDaemonGateToMatchReceipt(gate: Record<string, any>, receipt: Record<string, any>) {
  for (const [field, expectedValue] of Object.entries(receipt)) {
    if (ROLLING_DAEMON_GATE_FIELDS.has(field)) {
      expect(Array.isArray(gate[field])).toBe(true);
      expect(gate[field]).toEqual(expect.arrayContaining(expectedValue as any[]));
    } else {
      expect(gate[field]).toEqual(expectedValue);
    }
  }
}
...
test.describe('MGW-535 daemon launch health Playwright gate', () => {
  test('keeps the daemon launch validation gate fixture in parity with the manager', () => {
    ...
    expect(gate.supervisor_gap_receipts).toContain('data/hallucinate_multimodal_control/discovery/2026-07-08-hao-743-objective-gap-b023c8de5b69.md');
    expect(gate.supervisor_gap_receipts).toContain('data/hallucinate_multimodal_control/discovery/2026-07-08-hao-745-objective-gap-b023c8de5b69.md');
    expect(gate.supervisor_gap_receipts).toContain('data/hallucinate_multimodal_control/discovery/2026-07-08-hao-755-objective-gap-b023c8de5b69.md');
    ...
    expect(gate.hallucinate_backlog_receipts).toContain('data/hallucinate_multimodal_control/discovery/2026-07-08-hao-743-daemon-launch-health-gate.md');
    expect(gate.hallucinate_backlog_receipts).toContain('data/hallucinate_multimodal_control/discovery/2026-07-08-hao-745-daemon-launch-health-gate.md');
    expect(gate.hallucinate_backlog_receipts).toContain('data/hallucinate_multimodal_control/discovery/2026-07-08-hao-755-daemon-launch-health-gate.md');
  ...
  test('closes the MGW-590 daemon launch objective gap with the current Playwright gate', () => {
    const manager = new MCPDaemonManager();
    const fixture = JSON.parse(fs.readFileSync(MGW_590_GATE_FIXTURE, 'utf8'));
    const gates = manager.getDaemonLaunchValidationGates();
    ...
    expect(gate.packet_goals).toEqual(['VAIOS-G724', 'VAIOS-G728']);
    expect(gate.evidence_term).toBe('launch Playwright validation gate');
    expect(gate.gate_state).toBe('gate_closed_by_playwright_validation');
    expect(gate.supervisor_gap_receipt).toBe(
    ...
    expect(gate.discovery_receipts).toContain(gate.launch_gate_receipt);
    expect(gate.receipt_fixture).toBe('hallucinate_app/test/e2e/fixtures/mgw-590-daemon-launch-health-gate.json');
    expect(gate.todo_source).toEqual({
      file: 'implementation_plan/docs/18-swissknife-meta-glasses-display-widgets.todo.md',
      source_line: 3938
    });
    expect(gate.packet_sibling_task_id).toBe('MGW-589');
    expect(gate.packet_sibling_goal_id).toBe('VAIOS-G724');
    expect(gate.packet_sibling_gap_receipt).toBe(
  ...
  test('binds the VAI-536 objective gap receipt to the daemon launch health gate', () => {
    ...
    expectDaemonGateToMatchReceipt(vai536Gate, receipt);
  ...
  test('binds the rolling VAI daemon objective gap receipts through VAI-645 to the daemon launch Playwright gate', () => {
    ...
    const fixtures = [
      ...
      {
        taskId: 'VAI-652',
        fixturePath: VAI_652_GATE_FIXTURE,
        gapReceipt: 'data/virtual_ai_os/discovery/2026-07-05-vai-652-objective-gap-b023c8de5b69.md',
        launchReceipt: 'data/virtual_ai_os/discovery/2026-07-05-vai-652-daemon-launch-health-gate.md',
        daemonLaunchCommand: 'test ! -f hallucinate_app/package.json || npm --prefix hallucinate_app run test:e2e -- daemon-launch-health.spec.ts'
      },
      {
        taskId: 'VAI-654',
        fixturePath: VAI_654_GATE_FIXTURE,
        gapReceipt: 'data/virtual_ai_os/discovery/2026-07-05-vai-654-objective-gap-b023c8de5b69.md',
        launchReceipt: 'data/virtual_ai_os/discovery/2026-07-05-vai-654-daemon-launch-health-gate.md',
        daemonLaunchCommand: 'test ! -f hallucinate_app/package.json || npm --prefix hallucinate_app run test:e2e -- daemon-launch-health.spec.ts'
      },
      {
        taskId: 'VAI-656',
        fixturePath: VAI_656_GATE_FIXTURE,
        gapReceipt: 'data/virtual_ai_os/discovery/2026-07-05-vai-656-objective-gap-b023c8de5b69.md',
        launchReceipt: 'data/virtual_ai_os/discovery/2026-07-05-vai-656-daemon-launch-health-gate.md',
        daemonLaunchCommand: 'test ! -f hallucinate_app/package.json || npm --prefix hallucinate_app run test:e2e -- daemon-launch-health.spec.ts'
      },
      {
        taskId: 'VAI-658',
        fixturePath: VAI_658_GATE_FIXTURE,
        gapReceipt: 'data/virtual_ai_os/discovery/2026-07-05-vai-658-objective-gap-b023c8de5b69.md',
        launchReceipt: 'data/virtual_ai_os/discovery/2026-07-05-vai-658-daemon-launch-health-gate.md',
        daemonLaunchCommand: 'test ! -f hallucinate_app/package.json || npm --prefix hallucinate_app run test:e2e -- daemon-launch-health.spec.ts'
      },
      {
        taskId: 'VAI-660',
        fixturePath: VAI_660_GATE_FIXTURE,
    ...
    for (const fixture of fixtures) {
      ...
      expectDaemonGateToMatchReceipt(vaiGate, receipt);
      expect(receipt.schema).toBe('hallucinate_app.daemon_launch_validation_gate.v1');
      expect(receipt.receipt_schema).toBe('launch_readiness_receipt_v1');
  ...
  test('keeps the VAI-658 discovery receipt and objective heap aligned with the Playwright gate', () => {
    const manager = new MCPDaemonManager();
    const gates = manager.getDaemonLaunchValidationGates();
    const vai658Gate = gates.find((candidate: any) => candidate.task_id === 'VAI-658') as any;
    const receipt = fs.readFileSync(VAI_658_ATTEMPT_RECEIPT, 'utf8');
    const objectiveHeap = fs.readFileSync(OBJECTIVE_HEAP, 'utf8');

    expect(vai658Gate).toBeTruthy();
    expect(receipt).toContain('VAI-658 attempt 2 records the daemon launch Playwright validation gate');
    expect(receipt).toContain(vai658Gate.objective_gap_receipt);
    expect(receipt).toContain(vai658Gate.launch_gate_receipt);
    expect(receipt).toContain('hallucinate_app/test/e2e/fixtures/vai-657-mcp-dashboard-launch-gate.json');
    expect(receipt).toContain('swissknife/test/e2e/meta-glasses-virtual-os.spec.ts');
    ...
    expect(objectiveHeap).toContain(vai658Gate.receipt_fixture);
    expect(objectiveHeap).toContain(vai658Gate.launch_gate_receipt);
    expect(objectiveHeap).toContain('No additional child goals are needed because VAI-658 reuses the existing daemon launch');
  });

  test('keeps the VAI-660 discovery receipt and objective heap aligned with the Playwright gate', () => {
    const manager = new MCPDaemonManager();
    const gates = manager.getDaemonLaunchValidationGates();
    const vai660Gate = gates.find((candidate: any) => candidate.task_id === 'VAI-660') as any;
    const receipt = fs.readFileSync(VAI_660_ATTEMPT_RECEIPT, 'utf8');
    const objectiveHeap = fs.readFileSync(OBJECTIVE_HEAP, 'utf8');

    expect(vai660Gate).toBeTruthy();
    expect(receipt).toContain('VAI-660 attempt 1 records the daemon launch Playwright validation gate');
    expect(receipt).toContain(vai660Gate.objective_gap_receipt);
    expect(receipt).toContain(vai660Gate.launch_gate_receipt);
    expect(receipt).toContain('hallucinate_app/test/e2e/fixtures/vai-659-mcp-dashboard-launch-gate.json');
    expect(receipt).toContain('swissknife/test/e2e/meta-glasses-virtual-os.spec.ts');
