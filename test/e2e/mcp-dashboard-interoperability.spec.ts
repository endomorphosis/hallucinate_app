...
test.describe('MCP Dashboard Interoperability - VAIOS-G723 headless backend gate', () => {
  ...
  test('closes the HAO-754 objective gap with the shared VAIOS-G724/VAIOS-G728 launch packet gate', () => {
    ...
    const receipt = JSON.parse(fs.readFileSync(HAO_754_LAUNCH_GATE_FIXTURE, 'utf8'));
    const catalog = new MCPDaemonManager().getDashboardCapabilityCatalog();
    const launchGate = catalog.launch_validation_gates?.find((gate: any) => gate.task_id === 'HAO-754');

    expect(receipt.packet_sibling_task_id).toBe('HAO-755');
    expect(receipt.packet_sibling_goal_id).toBe('VAIOS-G728');
    expect(launchGate).toMatchObject({
