...
const VAI_653_ATTEMPT_1_LAUNCH_GATE_RECEIPT = path.join(
  REPO_ROOT,
  'data',
  'virtual_ai_os',
  'discovery',
  '2026-07-05-vai-653-attempt-1-launch-playwright-validation-gate.md'
);
...
test.describe('MCP Dashboard Interoperability - VAIOS-G723 headless backend gate', () => {
  ...
  test('closes the VAI-653 objective gap with the shared VAIOS-G724/VAIOS-G728 launch packet gate', () => {
    ...
    const attemptReceipt = fs.readFileSync(VAI_653_ATTEMPT_1_LAUNCH_GATE_RECEIPT, 'utf8');
    ...
    expect(objectiveHeap).toContain('VAI-653 attempt 1 validation');
