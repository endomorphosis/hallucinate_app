...
const HAO_718_LAUNCH_GATE_FIXTURE = path.join(__dirname, 'fixtures', 'hao-718-mcp-dashboard-launch-gate.json');
...
test.describe('MCP Dashboard Interoperability - VAIOS-G723 headless backend gate', () => {
  ...
  test('closes the HAO-718 objective gap with the current Hallucinate launch Playwright validation gate', () => {
    const receipt = JSON.parse(fs.readFileSync(HAO_718_LAUNCH_GATE_FIXTURE, 'utf8'));
    const launchGateReceipt = fs.readFileSync(path.join(
      REPO_ROOT,
      'data',
      'hallucinate_multimodal_control',
      'discovery',
      '2026-06-28-hao-718-mcp-dashboard-launch-gate.md'
    ), 'utf8');
    const objectiveGap = fs.readFileSync(path.join(
      REPO_ROOT,
      'data',
      'hallucinate_multimodal_control',
      'discovery',
      '2026-06-28-hao-718-objective-gap-3e00ad2a0074.md'
    ), 'utf8');
    const objectiveHeap = fs.readFileSync(MGW_OBJECTIVE_HEAP, 'utf8');
    const catalog = new MCPDaemonManager().getDashboardCapabilityCatalog();
    const serversByPackage = new Map(catalog.servers.map((server: any) => [server.server_package, server]));
    ...
    expect(receipt.schema).toBe('launch_readiness_receipt_v1');
    expect(receipt.task_id).toBe('HAO-718');
    expect(receipt.goal_id).toBe('VAIOS-G724');
    expect(receipt.goal_packet).toBe('goal_packet/launch/hallucinate_app/44dceea6bc53');
    expect(receipt.packet_goal_ids).toEqual(['VAIOS-G724', 'VAIOS-G728']);
    expect(receipt.source_gap_receipt).toBe(
      'data/hallucinate_multimodal_control/discovery/2026-06-28-hao-718-objective-gap-3e00ad2a0074.md'
    );
    expect(receipt.launch_gate_receipt).toBe(
    ...
    expect(objectiveHeap).toContain('HAO-718 proof');
    expect(objectiveHeap).toContain('hallucinate_app/test/e2e/fixtures/hao-718-mcp-dashboard-launch-gate.json');
    expect(objectiveHeap).toContain('data/hallucinate_multimodal_control/discovery/2026-06-28-hao-718-mcp-dashboard-launch-gate.md');
  });
});
