...
const VAI_560_LAUNCH_GATE_FIXTURE = path.join(__dirname, 'fixtures', 'vai-560-mcp-dashboard-launch-gate.json');
...
const VAI_558_LAUNCH_GATE_FIXTURE = path.join(__dirname, 'fixtures', 'vai-558-mcp-dashboard-launch-gate.json');
const VAI_558_OBJECTIVE_GAP_RECEIPT = path.join(
  REPO_ROOT,
  'data',
  'virtual_ai_os',
  'discovery',
  '2026-07-03-vai-558-objective-gap-3e00ad2a0074.md'
);
const VAI_558_LAUNCH_GATE_RECEIPT = path.join(
  REPO_ROOT,
  'data',
  'virtual_ai_os',
  'discovery',
  '2026-07-03-vai-558-mcp-dashboard-launch-gate.md'
);
const VAI_561_LAUNCH_GATE_FIXTURE = path.join(__dirname, 'fixtures', 'vai-561-mcp-dashboard-launch-gate.json');
const VAI_561_OBJECTIVE_GAP_RECEIPT = path.join(
  REPO_ROOT,
  'data',
  'virtual_ai_os',
  'discovery',
  '2026-07-03-vai-561-objective-gap-3e00ad2a0074.md'
);
const VAI_561_LAUNCH_GATE_RECEIPT = path.join(
  REPO_ROOT,
  'data',
  'virtual_ai_os',
  'discovery',
  '2026-07-03-vai-561-mcp-dashboard-launch-gate.md'
);
...
const VAI_560_OBJECTIVE_GAP_RECEIPT = path.join(
  REPO_ROOT,
  'data',
  'virtual_ai_os',
  'discovery',
  '2026-07-03-vai-560-objective-gap-7ea369464239.md'
);
const VAI_560_LAUNCH_GATE_RECEIPT = path.join(
  REPO_ROOT,
  'data',
  'virtual_ai_os',
  'discovery',
  '2026-07-03-vai-560-mcp-dashboard-launch-gate.md'
);
const VAI_560_HALLUCINATE_LAUNCH_GATE_RECEIPT = path.join(
  REPO_ROOT,
  'data',
  'hallucinate_multimodal_control',
  'discovery',
...
test.describe('MCP Dashboard Interoperability - VAIOS-G723 headless backend gate', () => {
  ...
  test('closes the VAI-558 objective gap with the current Hallucinate launch Playwright validation gate', () => {
    validateDashboardLaunchGateReceipt({
      fixturePath: VAI_558_LAUNCH_GATE_FIXTURE,
      launchGateReceiptPath: VAI_558_LAUNCH_GATE_RECEIPT,
      objectiveGapPath: VAI_558_OBJECTIVE_GAP_RECEIPT,
      taskId: 'VAI-558',
      sourceGapReceipt: 'data/virtual_ai_os/discovery/2026-07-03-vai-558-objective-gap-3e00ad2a0074.md',
      launchGateReceipt: 'data/virtual_ai_os/discovery/2026-07-03-vai-558-mcp-dashboard-launch-gate.md',
      receiptFixture: 'hallucinate_app/test/e2e/fixtures/vai-558-mcp-dashboard-launch-gate.json',
      heapProof: 'VAI-558 proof',
      gateState: 'gate_closed_by_playwright_validation'
    });
  });

  test('closes the VAI-561 objective gap with the current Hallucinate launch Playwright validation gate', () => {
    validateDashboardLaunchGateReceipt({
      fixturePath: VAI_561_LAUNCH_GATE_FIXTURE,
      launchGateReceiptPath: VAI_561_LAUNCH_GATE_RECEIPT,
      objectiveGapPath: VAI_561_OBJECTIVE_GAP_RECEIPT,
      taskId: 'VAI-561',
      sourceGapReceipt: 'data/virtual_ai_os/discovery/2026-07-03-vai-561-objective-gap-3e00ad2a0074.md',
      launchGateReceipt: 'data/virtual_ai_os/discovery/2026-07-03-vai-561-mcp-dashboard-launch-gate.md',
      receiptFixture: 'hallucinate_app/test/e2e/fixtures/vai-561-mcp-dashboard-launch-gate.json',
      heapProof: 'VAI-561 proof',
      gateState: 'gate_closed_by_playwright_validation'
    });
  });
  ...
  test('binds VAI-560 objective gap evidence to the VAIOS-G723 dashboard launch Playwright gate', () => {
    const receipt = JSON.parse(fs.readFileSync(VAI_560_LAUNCH_GATE_FIXTURE, 'utf8'));
    const launchGateReceipt = fs.readFileSync(VAI_560_LAUNCH_GATE_RECEIPT, 'utf8');
    const hallucinateLaunchGateReceipt = fs.readFileSync(VAI_560_HALLUCINATE_LAUNCH_GATE_RECEIPT, 'utf8');
    const objectiveGap = fs.readFileSync(VAI_560_OBJECTIVE_GAP_RECEIPT, 'utf8');
    const objectiveHeap = fs.readFileSync(MGW_OBJECTIVE_HEAP, 'utf8');
    const readinessDoc = fs.readFileSync(LAUNCH_READINESS_DOC, 'utf8');
    const catalog = new MCPDaemonManager().getDashboardCapabilityCatalog();
    const launchGate = catalog.launch_validation_gates?.find((gate: any) => gate.task_id === 'VAI-560');
    const serversByPackage = new Map(catalog.servers.map((server: any) => [server.server_package, server]));
    ...
    expect(launchGate.attempt).toBeUndefined();
    expect(launchGate.attempt_receipts).toBeUndefined();
    ...
    expect(objectiveHeap).toContain('VAI-560 proof');
    expect(objectiveHeap).toContain(receipt.receipt_fixture);
    expect(objectiveHeap).toContain(receipt.launch_gate_receipt);
    expect(objectiveHeap).toContain(receipt.hallucinate_backlog_receipt);
    expect(readinessDoc).toContain('VAI-560');
    expect(readinessDoc).toContain(receipt.receipt_fixture);
    expect(readinessDoc).toContain(receipt.launch_gate_receipt);
    expect(readinessDoc).toContain(receipt.hallucinate_backlog_receipt);
  });
