...
async function runTests() {
  ...
  console.log('\nTest 10: MGW-535, MGW-551, MGW-556, and MGW-565 daemon launch validation gates');
  ...
  const launchGateOk =
    ...
    launchGates.some(gate =>
      gate.task_id === 'MGW-565' &&
      gate.goal_id === 'VAIOS-G728' &&
      gate.supervisor_gap_receipt === 'data/meta_glasses_display_widgets/discovery/2026-07-02-mgw-565-objective-gap-b023c8de5b69.md' &&
      gate.launch_gate_receipt === 'data/meta_glasses_display_widgets/discovery/2026-07-02-mgw-565-daemon-launch-health-gate.md' &&
      gate.receipt_fixture === 'hallucinate_app/test/e2e/fixtures/mgw-565-daemon-launch-health-gate.json' &&
      gate.validation_commands?.includes('test ! -f hallucinate_app/package.json || npm --prefix hallucinate_app run test:e2e -- daemon-launch-health.spec.ts')
    ) &&
    launchGates.some(gate =>
  ...
  if (launchGateOk) {
    console.log('✅ MGW-535, MGW-551, MGW-556, and MGW-565 daemon launch validation gates are scanner-visible');
    testsPassed++;
  } else {
