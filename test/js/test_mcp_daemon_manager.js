...
async function runTests() {
  ...
  const catalogSpecificsOk =
    ...
    catalog.launch_validation_gates?.some(gate =>
      gate.task_id === 'HAO-729' &&
      gate.goal_id === 'VAIOS-G723' &&
      gate.supervisor_gap_receipt === 'data/hallucinate_multimodal_control/discovery/2026-06-30-hao-729-objective-gap-7ea369464239.md' &&
      gate.launch_gate_receipt === 'data/hallucinate_multimodal_control/discovery/2026-07-08-hao-729-mcp-dashboard-launch-gate.md' &&
      gate.receipt_fixture === 'hallucinate_app/test/e2e/fixtures/hao-729-mcp-dashboard-launch-gate.json' &&
      gate.attempt === 1 &&
      gate.attempt_receipts?.includes('data/hallucinate_multimodal_control/discovery/2026-07-08-hao-729-attempt-1-validation.md')
    ) &&
    catalog.launch_validation_gates?.some(gate =>
