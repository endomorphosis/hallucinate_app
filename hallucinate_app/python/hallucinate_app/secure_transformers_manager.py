"""
Secure Transformers Manager Module

Provides capability-based secure access to transformer operations
Integrates with UCAN authentication for decentralized auth
Implements proper error handling and access control
"""

import os
import json
import logging
import asyncio
import time
from datetime import datetime
from typing import Dict, List, Optional, Union, Any, Set

# Set up logging
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s"
)
logger = logging.getLogger("secure_transformers_manager")

# Try to import auth manager
try:
    from .auth import auth_manager, AuthManager
    has_auth = True
except ImportError:
    logger.warning("Auth module not available, using default empty implementation")
    has_auth = False
    
# Try to import transformers manager
try:
    from .ipfs_transformers import ipfs_transformers, IPFSTransformers
    has_transformers_manager = True
except ImportError:
    logger.warning("IPFS Transformers not available, using mock implementation")
    has_transformers_manager = False

# Define capability namespaces for transformer operations
TRANSFORMER_CAPABILITIES = {
    "LOAD": "transformer:load",
    "INFERENCE": "transformer:inference",
    "LIST": "transformer:list",
    "ADMIN": "transformer:admin",
}


class SecureTransformersManager:
    """
    Secure Transformers Manager providing capability-based access control
    """
    
    def __init__(self, resources=None, metadata=None):
        """
        Initialize the secure transformers manager
        
        Args:
            resources: Resources dictionary
            metadata: Metadata dictionary with configuration
        """
        self.resources = resources or {}
        self.metadata = metadata or {}
        
        # Use resources if provided, otherwise use default instances
        self.auth = self.resources.get("auth") or (auth_manager if has_auth else None)
        
        if has_transformers_manager:
            self.transformers_manager = self.resources.get("transformers") or ipfs_transformers
        else:
            self.transformers_manager = None
        
        self.initialized = False
        
        # Cache for tracking loaded models and their capabilities
        self.model_access_cache = {}
        self.model_load_requests = {}
        
        # Operational stats
        self.stats = {
            "access_granted": 0,
            "access_denied": 0,
            "models_loaded": 0,
            "inferences_run": 0,
            "last_request": None
        }
        
        # Resource usage monitoring
        self.resource_usage = {
            "by_model": {},
            "by_user": {}
        }
        
        logger.info("Secure Transformers Manager initialized")
    
    async def init(self) -> bool:
        """
        Initialize the secure transformers manager
        
        Returns:
            bool: True if initialization successful
        """
        try:
            # Ensure auth manager is initialized
            if self.auth and hasattr(self.auth, "init"):
                if not getattr(self.auth, "initialized", False):
                    await self.auth.init()
            
            # Initialize underlying transformers manager if needed
            if self.transformers_manager and hasattr(self.transformers_manager, "init"):
                await self.transformers_manager.init()
            
            self.initialized = True
            return True
        except Exception as e:
            logger.error(f"Failed to initialize secure transformers manager: {e}")
            return False
    
    async def load_model(self, model_id: str, options: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Securely load a transformer model with capability verification
        
        Args:
            model_id: Model identifier to load
            options: Loading options including auth_token for authorization
            
        Returns:
            Dict: Load result
        """
        if not self.initialized:
            raise RuntimeError("Secure transformers manager not initialized. Call init() first")
        
        self.stats["last_request"] = {
            "action": "load_model",
            "model_id": model_id,
            "timestamp": datetime.now().isoformat()
        }
        
        options = options or {}
        
        try:
            auth_token = options.get("auth_token")
            
            if not auth_token:
                raise ValueError("Authentication token required for model loading")
            
            # Verify capability token for model access
            capability_string = f"{TRANSFORMER_CAPABILITIES['LOAD']}:{model_id}"
            is_authorized = False
            
            if self.auth:
                is_authorized = await self.auth.verify_capability(auth_token, capability_string)
                
                if not is_authorized:
                    # Check if a broader transformer:load:* capability exists
                    wildcard_authorized = await self.auth.verify_capability(auth_token, f"{TRANSFORMER_CAPABILITIES['LOAD']}:*")
                    
                    if not wildcard_authorized:
                        self.stats["access_denied"] += 1
                        logger.warning(f"Unauthorized transformer model load attempt for {model_id}")
                        raise PermissionError(f"Not authorized to load transformer model {model_id}")
                    else:
                        is_authorized = True
            else:
                # If auth is not available, allow access but log warning
                logger.warning("Auth module not available, allowing model load without verification")
                is_authorized = True
            
            # Track access
            self.stats["access_granted"] += 1
            
            # Check if model is already being loaded
            if model_id in self.model_load_requests:
                logger.info(f"Model {model_id} is already being loaded, waiting for completion")
                return await self.model_load_requests[model_id]
            
            # Create a future for this load request
            future = asyncio.Future()
            self.model_load_requests[model_id] = future
            
            try:
                # Call underlying transformers manager
                task = options.get("task")
                
                if self.transformers_manager and hasattr(self.transformers_manager, "load_model"):
                    success = await self.transformers_manager.load_model(model_id, task)
                    
                    if success:
                        # Get active model info
                        active_model_id = getattr(self.transformers_manager, "active_model_id", model_id)
                        
                        # Update cache with successful load
                        self.model_access_cache[model_id] = {
                            "loaded_at": datetime.now().isoformat(),
                            "loaded_by": self._extract_principal_from_token(auth_token),
                            "token": auth_token,
                            "last_used": datetime.now().isoformat(),
                            "task": task,
                            "active_model_id": active_model_id
                        }
                        
                        # Set future result
                        result = {
                            "success": True,
                            "model_id": model_id,
                            "task": task
                        }
                        future.set_result(result)
                        
                        # Update stats
                        self.stats["models_loaded"] += 1
                        self._update_resource_usage("load", model_id, options)
                        
                        return result
                    else:
                        # Set future with error
                        error_result = {
                            "success": False,
                            "model_id": model_id,
                            "error": "Failed to load model"
                        }
                        future.set_result(error_result)
                        return error_result
                else:
                    # Mock implementation if no transformers manager available
                    mock_result = {
                        "success": True,
                        "model_id": model_id,
                        "task": task,
                        "mock": True,
                        "message": "Mock model loaded (no real transformers manager available)"
                    }
                    
                    # Update cache with mock load
                    self.model_access_cache[model_id] = {
                        "loaded_at": datetime.now().isoformat(),
                        "loaded_by": self._extract_principal_from_token(auth_token),
                        "token": auth_token,
                        "last_used": datetime.now().isoformat(),
                        "task": task,
                        "mock": True
                    }
                    
                    # Set future result
                    future.set_result(mock_result)
                    
                    # Update stats
                    self.stats["models_loaded"] += 1
                    self._update_resource_usage("load", model_id, options)
                    
                    return mock_result
            except Exception as e:
                # Set future with error
                error_result = {
                    "success": False,
                    "model_id": model_id,
                    "error": str(e)
                }
                future.set_result(error_result)
                return error_result
            finally:
                # Remove from pending requests
                if model_id in self.model_load_requests:
                    del self.model_load_requests[model_id]
        except Exception as e:
            logger.error(f"Secure transformer model load failed for {model_id}: {e}")
            return {
                "success": False,
                "model_id": model_id,
                "error": str(e)
            }
    
    async def run_inference(self, input_text: str, options: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Securely run inference with capability verification
        
        Args:
            input_text: Input text for inference
            options: Inference options including auth_token for authorization
            
        Returns:
            Dict: Inference result
        """
        if not self.initialized:
            raise RuntimeError("Secure transformers manager not initialized. Call init() first")
        
        options = options or {}
        model_id = options.get("model_id")
        
        self.stats["last_request"] = {
            "action": "run_inference",
            "model_id": model_id,
            "timestamp": datetime.now().isoformat()
        }
        
        try:
            auth_token = options.get("auth_token")
            
            if not auth_token:
                raise ValueError("Authentication token required for transformer inference")
            
            # Verify capability token for transformer inference
            capability_string = f"{TRANSFORMER_CAPABILITIES['INFERENCE']}:{model_id}" if model_id else f"{TRANSFORMER_CAPABILITIES['INFERENCE']}:*"
            is_authorized = False
            
            if self.auth:
                is_authorized = await self.auth.verify_capability(auth_token, capability_string)
                
                if not is_authorized and model_id:
                    # Check if a broader transformer:inference:* capability exists
                    wildcard_authorized = await self.auth.verify_capability(auth_token, f"{TRANSFORMER_CAPABILITIES['INFERENCE']}:*")
                    
                    if not wildcard_authorized:
                        self.stats["access_denied"] += 1
                        logger.warning(f"Unauthorized transformer inference attempt for {model_id}")
                        raise PermissionError(f"Not authorized to run inference on transformer model {model_id}")
                    else:
                        is_authorized = True
            else:
                # If auth is not available, allow access but log warning
                logger.warning("Auth module not available, allowing inference without verification")
                is_authorized = True
            
            # Track access
            self.stats["access_granted"] += 1
            
            # If model_id is specified, make sure it's loaded first
            if model_id and model_id not in self.model_access_cache:
                load_options = {
                    "auth_token": auth_token,
                    "task": options.get("task"),
                    "user_id": options.get("user_id")
                }
                load_result = await self.load_model(model_id, load_options)
                if not load_result.get("success", False):
                    return {
                        "success": False,
                        "model_id": model_id,
                        "error": f"Failed to load model {model_id}"
                    }
            
            # Call underlying transformers manager implementation
            if self.transformers_manager and hasattr(self.transformers_manager, "run_inference"):
                # Extract inference parameters
                params = options.get("params", {})
                task = options.get("task")
                
                # Run inference
                result = await self.transformers_manager.run_inference(
                    input_text=input_text,
                    model_id=model_id,
                    task=task,
                    params=params
                )
                
                # Update stats
                self.stats["inferences_run"] += 1
                self._update_resource_usage("inference", model_id or "default", options)
                
                # Update last used timestamp if model in cache
                used_model_id = model_id or result.get("model_id")
                if used_model_id and used_model_id in self.model_access_cache:
                    self.model_access_cache[used_model_id]["last_used"] = datetime.now().isoformat()
                
                return result
            else:
                # Mock implementation
                mock_result = {
                    "success": True,
                    "model_id": model_id or "default-model",
                    "input": input_text,
                    "mock": True,
                    "results": "This is a mock inference result. In a real implementation, this would be the model output."
                }
                
                # Update stats
                self.stats["inferences_run"] += 1
                self._update_resource_usage("inference", model_id or "default", options)
                
                return mock_result
        except Exception as e:
            logger.error(f"Secure transformer inference failed: {e}")
            return {
                "success": False,
                "model_id": model_id,
                "error": str(e)
            }
    
    async def list_models(self, options: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Securely list loaded transformer models with capability verification
        
        Args:
            options: List options including auth_token for authorization
            
        Returns:
            Dict: List of loaded models
        """
        if not self.initialized:
            raise RuntimeError("Secure transformers manager not initialized. Call init() first")
        
        self.stats["last_request"] = {
            "action": "list_models",
            "timestamp": datetime.now().isoformat()
        }
        
        options = options or {}
        
        try:
            auth_token = options.get("auth_token")
            
            if not auth_token:
                raise ValueError("Authentication token required for model listing")
            
            # Verify capability token for model listing
            capability_string = f"{TRANSFORMER_CAPABILITIES['LIST']}:*"
            is_authorized = False
            
            if self.auth:
                is_authorized = await self.auth.verify_capability(auth_token, capability_string)
                
                if not is_authorized:
                    self.stats["access_denied"] += 1
                    logger.warning("Unauthorized transformer model listing attempt")
                    raise PermissionError("Not authorized to list transformer models")
            else:
                # If auth is not available, allow access but log warning
                logger.warning("Auth module not available, allowing model listing without verification")
                is_authorized = True
            
            # Track access
            self.stats["access_granted"] += 1
            
            # Return the model cache
            result = {}
            for model_id, model_info in self.model_access_cache.items():
                result[model_id] = {
                    "model_id": model_id,
                    "loaded_at": model_info.get("loaded_at"),
                    "loaded_by": model_info.get("loaded_by"),
                    "last_used": model_info.get("last_used"),
                    "task": model_info.get("task"),
                    "active": model_info.get("active_model_id") == model_id
                }
            
            return {
                "success": True,
                "models": result,
                "count": len(result)
            }
        except Exception as e:
            logger.error(f"Secure transformer model listing failed: {e}")
            return {
                "success": False,
                "error": str(e)
            }
    
    def _extract_principal_from_token(self, token: str) -> str:
        """
        Extract principal ID from auth token (simplified)
        
        Args:
            token: Auth token
            
        Returns:
            str: Principal ID
        """
        # In a real implementation, this would decode the UCAN token
        # For now, we'll just return a placeholder value
        return "principal:unknown"
    
    def _update_resource_usage(self, operation: str, model_id: str, 
                             options: Dict[str, Any] = None) -> None:
        """
        Update resource usage tracking
        
        Args:
            operation: Operation type
            model_id: Model ID
            options: Operation options
        """
        options = options or {}
        
        # Initialize model tracking if needed
        if model_id not in self.resource_usage["by_model"]:
            self.resource_usage["by_model"][model_id] = {
                "loads": 0,
                "inferences": 0,
                "last_access": None
            }
        
        # Initialize user tracking if options has user info
        user_id = options.get("user_id", "anonymous")
        if user_id not in self.resource_usage["by_user"]:
            self.resource_usage["by_user"][user_id] = {
                "loads": 0,
                "inferences": 0,
                "models": set()
            }
        
        # Update counters based on operation
        if operation == "load":
            self.resource_usage["by_model"][model_id]["loads"] += 1
            self.resource_usage["by_user"][user_id]["loads"] += 1
            self.resource_usage["by_user"][user_id]["models"].add(model_id)
        elif operation == "inference":
            self.resource_usage["by_model"][model_id]["inferences"] += 1
            self.resource_usage["by_user"][user_id]["inferences"] += 1
        
        # Update last access timestamp
        self.resource_usage["by_model"][model_id]["last_access"] = datetime.now().isoformat()
    
    async def get_stats(self, options: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Get module statistics
        
        Args:
            options: Options for stats retrieval including auth_token
            
        Returns:
            Dict: Module statistics
        """
        if not self.initialized:
            raise RuntimeError("Secure transformers manager not initialized. Call init() first")
        
        options = options or {}
        
        try:
            auth_token = options.get("auth_token")
            
            if not auth_token and self.auth:
                raise ValueError("Authentication token required for stats access")
            
            # Verify capability token for admin access
            is_authorized = True
            if self.auth:
                capability_string = f"{TRANSFORMER_CAPABILITIES['ADMIN']}:stats"
                is_authorized = await self.auth.verify_capability(auth_token, capability_string)
                
                if not is_authorized:
                    logger.warning("Unauthorized stats access attempt")
                    raise PermissionError("Not authorized to access module statistics")
            
            # Return copy of stats
            top_models = self._get_top_models(5)
            
            # Need to convert sets to lists for serialization
            user_stats = {}
            for user_id, stats in self.resource_usage["by_user"].items():
                user_stats[user_id] = {
                    "loads": stats["loads"],
                    "inferences": stats["inferences"],
                    "models": list(stats["models"])
                }
            
            return {
                **self.stats,
                "model_count": len(self.model_access_cache),
                "resource_usage": {
                    "model_count": len(self.resource_usage["by_model"]),
                    "user_count": len(self.resource_usage["by_user"]),
                    "total_inferences": self.stats["inferences_run"],
                    "top_models": top_models,
                    "by_user": user_stats
                }
            }
        except Exception as e:
            logger.error(f"Failed to get secure transformers manager stats: {e}")
            return {
                "success": False,
                "error": str(e)
            }
    
    def _get_top_models(self, count: int = 5) -> List[Dict[str, Any]]:
        """
        Get top N models by inference count
        
        Args:
            count: Number of models to return
            
        Returns:
            List: Top models
        """
        # Sort models by inference count
        sorted_models = sorted(
            self.resource_usage["by_model"].items(),
            key=lambda x: x[1]["inferences"],
            reverse=True
        )
        
        # Take top N models
        top_models = sorted_models[:count]
        
        # Format results
        return [
            {
                "model_id": model_id,
                "inferences": stats["inferences"],
                "loads": stats["loads"],
                "last_access": stats["last_access"]
            }
            for model_id, stats in top_models
        ]
    
    def test(self) -> Dict[str, Any]:
        """
        Run tests on the secure transformers manager
        
        Returns:
            Dict: Test results
        """
        logger.info("Testing secure transformers manager")
        
        try:
            # Create asyncio event loop for async tests
            loop = asyncio.get_event_loop()
            
            test_results = {
                "success": True,
                "module": "secure_transformers_manager",
                "initialization": False,
                "capability_verification": False,
                "model_operations": {
                    "load": False,
                    "inference": False,
                    "list": False
                },
                "stats_tracking": False
            }
            
            # Test initialization if not already initialized
            if not self.initialized:
                init_result = loop.run_until_complete(self.init())
                test_results["initialization"] = init_result
            else:
                test_results["initialization"] = True
            
            if test_results["initialization"]:
                # Create test principals and capabilities for testing if auth is available
                admin_token = "mock-admin-token"
                load_token = "mock-load-token"
                inference_token = "mock-inference-token"
                
                if self.auth:
                    # Create test user if not exists
                    if "test-user" not in self.auth.principals:
                        loop.run_until_complete(self.auth.create_principal("test-user"))
                    
                    # Issue capabilities
                    admin_token_result = loop.run_until_complete(
                        self.auth.issue_capability("root", "test-user", {
                            "can": TRANSFORMER_CAPABILITIES["ADMIN"],
                            "with": "*"
                        })
                    )
                    admin_token = admin_token_result["token"]
                    
                    load_token_result = loop.run_until_complete(
                        self.auth.issue_capability("root", "test-user", {
                            "can": TRANSFORMER_CAPABILITIES["LOAD"],
                            "with": "*"
                        })
                    )
                    load_token = load_token_result["token"]
                    
                    inference_token_result = loop.run_until_complete(
                        self.auth.issue_capability("root", "test-user", {
                            "can": TRANSFORMER_CAPABILITIES["INFERENCE"],
                            "with": "*"
                        })
                    )
                    inference_token = inference_token_result["token"]
                
                # Save original transformers manager
                original_transformers_manager = self.transformers_manager
                
                try:
                    # Create a mock transformers manager class for testing
                    class MockTransformersManager:
                        def __init__(self):
                            self.active_model_id = None
                            
                        async def init(self):
                            return True
                        
                        async def load_model(self, model_id, task=None):
                            self.active_model_id = model_id
                            return True
                        
                        async def run_inference(self, input_text, model_id=None, task=None, params=None):
                            return {
                                "model_id": model_id or self.active_model_id or "default-model",
                                "input": input_text,
                                "results": "Mock inference result"
                            }
                    
                    # Set mock transformers manager
                    self.transformers_manager = MockTransformersManager()
                    
                    # Test capability verification by trying with invalid token
                    try:
                        # This should fail with invalid token
                        await self.load_model("test-model", {"auth_token": "invalid-token"})
                        test_results["capability_verification"] = False
                    except (PermissionError, ValueError):
                        # Expected failure is good
                        test_results["capability_verification"] = True
                    
                    # If auth not available, assume verification passes
                    if not self.auth:
                        test_results["capability_verification"] = True
                    
                    # Test model operations
                    try:
                        # Test load model
                        load_result = await self.load_model("test-model", {
                            "auth_token": load_token,
                            "user_id": "test-user",
                            "task": "text-generation"
                        })
                        test_results["model_operations"]["load"] = load_result.get("success", False)
                        
                        # Test inference
                        inference_result = await self.run_inference("Test input", {
                            "auth_token": inference_token,
                            "user_id": "test-user",
                            "model_id": "test-model"
                        })
                        test_results["model_operations"]["inference"] = "results" in inference_result
                        
                        # Test list models
                        list_result = await self.list_models({
                            "auth_token": admin_token
                        })
                        test_results["model_operations"]["list"] = list_result.get("success", False)
                        
                        # Test stats
                        stats = await self.get_stats({
                            "auth_token": admin_token
                        })
                        test_results["stats_tracking"] = (
                            isinstance(stats, dict) and
                            "access_granted" in stats and
                            "inferences_run" in stats
                        )
                    except Exception as e:
                        logger.error(f"Model operations test failed: {e}")
                finally:
                    # Restore original transformers manager
                    self.transformers_manager = original_transformers_manager
            
            # Overall success
            test_results["success"] = (
                test_results["initialization"] and
                (test_results["capability_verification"] or not self.auth) and
                all(test_results["model_operations"].values()) and
                test_results["stats_tracking"]
            )
            
            return test_results
        except Exception as e:
            logger.error(f"Secure transformers manager test failed: {e}")
            return {
                "success": False,
                "module": "secure_transformers_manager",
                "error": str(e)
            }


# Create default instance
secure_transformers_manager = SecureTransformersManager()