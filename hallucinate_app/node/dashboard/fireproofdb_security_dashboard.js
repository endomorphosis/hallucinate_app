/**
 * FireproofDB Security Dashboard Component
 * 
 * Provides UI for monitoring and managing FireproofDB database security
...
import { secureFireproofdbManager, FIREPROOFDB_CAPABILITIES } from '../secure_fireproofdb_manager.js';
import { authManager } from '../auth.js';

class FireproofDBSecurityDashboard {
  /**
   * Create a new FireproofDB security dashboard component
  ...
  constructor(options = {}) {
    ...
    this.secureFireproofdb = this.resources.secureFireproofdb || secureFireproofdbManager;
    ...
    // Setup container
    this.container.classList.add('fireproofdb-security-dashboard');
    this.container.classList.add(`theme-${this.config.theme}`);
    
    // Create main dashboard sections
    this._createDashboardLayout();
    
    console.log('FireproofDB Security Dashboard initialized');
  }
  ...
  async init() {
    try {
      ...
      // Ensure secure FireproofDB manager is initialized
      if (!this.secureFireproofdb.initialized) {
        await this.secureFireproofdb.init();
      }
      ...
      console.error('Failed to initialize FireproofDB security dashboard:', error);
  ...
  render() {
    if (!this.initialized) {
      console.warn('FireproofDB security dashboard not initialized. Call init() first');
      return;
    }
  ...
  async refresh() {
    ...
    try {
      ...
      console.error('Failed to refresh FireproofDB security dashboard:', error);
      if (this.callbacks.onError) {
        this.callbacks.onError(error);
      }
    }
  ...
  async createCapability(principal, capability, resource) {
    if (!this.initialized) {
      throw new Error('FireproofDB security dashboard not initialized. Call init() first');
    }
  ...
  selectDatabase(dbName) {
    ...
    this._renderDatabaseDetails(dbName);
    
    // Call callback if provided
    if (this.callbacks.onDatabaseSelected) {
      this.callbacks.onDatabaseSelected(dbName);
    }
  }
  ...
  _createDashboardLayout() {
    ...
    headerSection.className = 'fireproofdb-security-header';
    headerSection.innerHTML = `
      <h2>FireproofDB Security Dashboard</h2>
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <select id="capability-select">
              ...
              ${Object.values(FIREPROOFDB_CAPABILITIES).map(cap => `<option value="${cap}">${cap}</option>`).join('')}
            </select>
          </div>
          <div class="form-group">
            <label for="resource-input">Resource:</label>
            <input type="text" id="resource-input" placeholder="Database name">
  ...
  async _refreshData() {
    try {
      ...
      // Fetch stats
      this.stats = await this.secureFireproofdb.getStats({ authToken: adminToken.token });
      
      // Fetch databases
      const databasesResult = await this.secureFireproofdb.listDatabases({ authToken: adminToken.token });
  ...
  _renderStats() {
    ...
    const statsHtml = `
      <div class="stat-grid">
        ...
        <div class="stat-item">
          <div class="stat-value">${this.stats.databases_deleted || 0}</div>
          <div class="stat-label">Deleted</div>
        </div>
        <div class="stat-item">
          <div class="stat-value">${this.stats.document_writes || 0}</div>
          <div class="stat-label">Writes</div>
        </div>
        <div class="stat-item">
          <div class="stat-value">${this.stats.document_reads || 0}</div>
          <div class="stat-label">Reads</div>
        </div>
        <div class="stat-item">
          <div class="stat-value">${this.stats.queries_performed || 0}</div>
  ...
  _renderTopResources() {
    ...
    const { top_databases: topDatabases } = this.stats.resource_usage;
    ...
    const topDbsHtml = topDatabases.map(db => {
      return `
        <div class="resource-item" data-name="${db.name}">
          <div class="resource-name" title="${db.name}">${db.name}</div>
    ...
    resourceItems.forEach(item => {
      item.addEventListener('click', () => {
        const dbName = item.dataset.name;
        if (dbName) {
  ...
  _renderDatabases() {
    ...
    const databasesArray = Array.isArray(this.databases) ? 
      this.databases : 
      Object.values(this.databases);
    
    // Sort databases by last used timestamp (most recent first)
    const sortedDatabases = databasesArray
      .sort((a, b) => {
        const timeA = new Date(a.last_used || a.created_at || 0).getTime();
        const timeB = new Date(b.last_used || b.created_at || 0).getTime();
        return timeB - timeA;
      })
      .slice(0, this.config.maxDatabasesToShow);
    
    const databasesHtml = sortedDatabases.map(db => {
      const dbName = db.name;
      const isSelected = dbName === this.selectedDatabase;
      return `
        <div class="database-item ${isSelected ? 'selected' : ''}" data-name="${dbName}">
          <div class="database-name" title="${dbName}">${dbName}</div>
          <div class="database-info">
            <span class="database-size">${db.doc_count || 0} docs</span>
            <span class="database-date">${this._formatDate(db.created_at)}</span>
          </div>
        </div>
      `;
    }).join('');
   ...
   * @param {string} dbName Database name
   */
  _renderDatabaseDetails(dbName) {
    const detailsContainer = this.container.querySelector('.database-details');
    if (!detailsContainer) return;
    
    // Find database in our data
    const database = Array.isArray(this.databases) ? 
      this.databases.find(db => db.name === dbName) : 
    ...
    const dbCapabilities = Object.entries(this.capabilities)
      .filter(([, caps]) => {
        ...
        return caps.some(cap => {
          ...
          return resource === dbName || resource === '*';
        });
      })
    ...
    const capabilitiesHtml = dbCapabilities.length > 0 ? 
      dbCapabilities.map(([principal, caps]) => {
        ...
        const capsList = caps
          ...
          .map(cap => {
            // Extract capability type from full capability string
            const capType = cap.can.split(':')[1] || cap.can;
            return `<span class="cap-item cap-${capType}">${capType}</span>`;
          })
          .join('');
    ...
    if (this.stats && this.stats.resource_usage && this.stats.resource_usage.by_database) {
      usageStats = this.stats.resource_usage.by_database[dbName] || {};
    }
    
    detailsContainer.innerHTML = `
      <h4 class="db-detail-name" title="${dbName}">${dbName}</h4>
      ...
      <div class="db-detail-section">
        ...
        <div class="db-detail-props">
          <div class="db-detail-prop">
            ...
            <span class="prop-value">${this._formatDate(database.created_at)}</span>
          </div>
          <div class="db-detail-prop">
            <span class="prop-label">Created By:</span>
            <span class="prop-value">${database.created_by || 'unknown'}</span>
          </div>
          <div class="db-detail-prop">
            <span class="prop-label">Document Count:</span>
            <span class="prop-value">${database.doc_count || 0}</span>
          </div>
          <div class="db-detail-prop">
            <span class="prop-label">Last Used:</span>
            <span class="prop-value">${this._formatDate(database.last_used)}</span>
          </div>
        </div>
      </div>
      ...
      <div class="db-detail-section">
        ...
        <div class="db-detail-stats">
          ...
          <div class="db-stat">
            ...
            <span class="stat-value">${usageStats.queries || 0}</span>
          </div>
          <div class="db-stat">
            <span class="stat-label">Exports:</span>
            <span class="stat-value">${usageStats.exports || 0}</span>
          </div>
          <div class="db-stat">
            <span class="stat-label">Imports:</span>
            <span class="stat-value">${usageStats.imports || 0}</span>
          </div>
          <div class="db-stat">
            <span class="stat-label">Syncs:</span>
            <span class="stat-value">${usageStats.syncs || 0}</span>
          </div>
        </div>
      </div>
      ...
      <div class="db-detail-actions">
        <button class="add-capability-button" data-resource="${dbName}">
  ...
  _renderPrincipalDetails(principalId) {
    ...
    // Filter capabilities related to FireproofDB
    const fireproofdbCapabilities = principalCapabilities.filter(cap => 
      cap.can && cap.can.startsWith('fireproofdb:')
    );
    
    // Group capabilities by type
    const groupedCapabilities = {};
    fireproofdbCapabilities.forEach(cap => {
      const capabilityType = cap.can.split(':')[1] || cap.can; // Extract type from full capability
      if (!groupedCapabilities[capabilityType]) {
        groupedCapabilities[capabilityType] = [];
      }
      groupedCapabilities[capabilityType].push(cap);
    });
    
    // Generate capabilities HTML
    const capabilitiesHtml = Object.keys(groupedCapabilities).length > 0 ? 
      Object.entries(groupedCapabilities).map(([capabilityType, caps]) => {
        ...
        return `
          <div class="principal-capability-group">
            <h6 class="capability-type">${capabilityType}</h6>
      ...
      '<p>No FireproofDB capabilities assigned</p>';
    ...
    detailsContainer.innerHTML = `
      ...
      <div class="principal-detail-section">
        <h5>FireproofDB Usage Statistics</h5>
      ...
      <div class="principal-detail-section">
        <h5>FireproofDB Capabilities</h5>
  ...
  _renderCapabilities() {
    ...
    // Flatten capabilities for display, filtering to only FireproofDB capabilities
    const flatCapabilities = [];
    Object.entries(this.capabilities).forEach(([principal, caps]) => {
      caps.forEach(cap => {
        if (cap.can && cap.can.startsWith('fireproofdb:')) {
    ...
    const capabilitiesHtml = limitedCapabilities.map(cap => {
      ...
      // Extract type from full capability string
      const capabilityType = cap.capability.split(':')[1] || cap.capability;
      ...
      return `
        <div class="capability-item ${isExpired ? 'expired' : ''}" data-id="${cap.id}" data-principal="${cap.principal}">
          ...
          <div class="capability-type cap-${capabilityType}">${capabilityType}</div>
    ...
    capabilitiesList.innerHTML = capabilitiesHtml || '<div class="capability-loading">No FireproofDB capabilities available</div>';
  ...
  _filterDatabases(searchTerm) {
    ...
    databaseItems.forEach(item => {
      ...
      if (dbName.toLowerCase().includes(searchTermLower)) {
        item.style.display = '';
      } else {
...
export { FireproofDBSecurityDashboard };
export default FireproofDBSecurityDashboard;